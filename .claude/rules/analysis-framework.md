...
**Single-biennium** (`just pipeline 2025-26`): phases 01-25 + 02b + 06b + 07b in order. Phases 02b, 06, 06b, 07b, 08, 16-23 gracefully skip when prerequisites are missing (no R, no bill texts, biennium out of range for SM/DIME).

EDA → PCA → EGA → MCA → Bill Text → IRT → 2D IRT → Bifactor IRT → Hierarchical IRT → Hierarchical 2D IRT → PPC → UMAP → Clustering → LCA → Network → Bipartite → Indices → Beta-Binomial → Prediction → W-NOMINATE → External Validation → DIME → TSA → TBIP → Issue IRT → Model Legislation → Synthesis → Profiles
...
- **MCMC (all models)**: nutpie Rust NUTS sampler — single process, Rust threads for parallel chains (ADR-0051, ADR-0053). Graph-building functions (`build_per_chamber_graph()`, `build_joint_graph()`, `build_irt_graph()`) return PyMC models without sampling. Sampling functions compile with `nutpie.compile_pymc_model()` and sample with `nutpie.sample()`. Initialization via `analysis/init_strategy.py`: `--init-strategy {auto,irt-informed,pca-informed,2d-dim1}` (ADR-0107). Auto prefers 1D IRT, falls back to PCA. **Phase 06 (2D IRT) defaults to `pca-informed`** to avoid horseshoe contamination from confounded 1D scores (see `docs/canonical-ideal-points.md`). `jitter_rvs` excludes the initialized variable. Robustness flags `--dim1-prior`, `--horseshoe-remediate` are research-only; production pipelines use **2-layer canonical routing** (ADR-0129): (1) horseshoe detection, (2) tiered convergence (H2D → flat 2D → 1D). Layer 1 (manual PCA overrides) removed — Fisher's LDA in Phase 02 handles axis instability automatically. W-NOMINATE gate removed (ADR-0127: shares same variance-ordering vulnerability as PCA). Phase 16 is standalone validation.
...
- **PCA axis instability in Senate sessions (RESOLVED, ADR-0118, ADR-0129):** Fisher's LDA ideology projection in Phase 02 finds the optimal party-separating direction across all PCs automatically, producing `ideology_score` and `establishment_score` columns. This replaces manual PCA overrides (`pca_overrides.yaml`, deprecated) and `detect_ideology_pc()`. See `docs/lda-ideology-projection.md`.
