...
- `@pytest.mark.scraper` — scraper pipeline tests (11 files, ~624 tests)
...
- `tests/test_db_hook.py` — post-scrape PostgreSQL hook: subprocess invocation, env vars, fail-soft (FileNotFoundError, timeout, non-zero exit), CLI --auto-load integration for all 3 CLIs (~28 tests)
...
- `tests/test_irt.py` — IRT data prep, anchor selection, sign validation, sensitivity, forest, paradox detection, convergence diagnostics, posterior extraction, equating, identification strategies, robustness flags, PC2-filtered vote selection, dim1-prior flag (~137 tests)
...
- `tests/test_hierarchical.py` — hierarchical data prep, model structure, variance decomposition, small-group warning, joint ordering, rescaling fallback, Independent exclusion, sign convention fix, bill-matching, adaptive priors, dim1-prior Potential (~57 tests)
...
- `tests/test_django_api.py` — REST API (DB4): health check, 7 resource endpoints (list/detail), filtering, pagination, schema validators (semicolon parsing, nullable fields), nested votes in rollcall detail (~64 tests). Marked `@pytest.mark.web` + `@pytest.mark.django_db`.
