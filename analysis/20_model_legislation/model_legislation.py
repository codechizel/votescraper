...
def _get_bill_text_data():  # type: ignore[no-untyped-def]
    ...
    if _bill_text_data is None:
        try:
            ...
            sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "18_bill_text"))
...
def main(argv: list[str] | None = None) -> None:
    ...
    with RunContext(
        ...
        analysis_name="20_model_legislation",
        params=vars(args),
        results_root=results_root,
        primer=MODEL_LEGISLATION_PRIMER,
        run_id=args.run_id,
    ) as ctx:
        print_header("Model Legislation Detection (Phase 20)")
