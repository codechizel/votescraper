...
def _load_sm_correlations(results_root: Path, run_id: str | None) -> dict | None:
    ...
    try:
        sm_dir = resolve_upstream_dir("17_external_validation", results_root, run_id)
...
def main() -> None:
    ...
    # Check DIME data file exists
    if not Path(DIME_CACHE_PATH).exists():
        print(f"[Phase 18] Skipping: DIME data not found ({DIME_CACHE_PATH})")
        return
