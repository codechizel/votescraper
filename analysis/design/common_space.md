...
- **`_SLUG_ALIASES`:** Maps data slugs (KS Legislature encoding) to OCD-mapped slugs (OpenStates encoding) when they differ due to nickname/spelling variants. 13 entries (e.g., `rep_roth_charlie_1` → `rep_roth_charles_1`). Applied before cross-chamber expansion so both variants and their chamber counterparts resolve correctly.
