...
just test                                    # → uv run pytest tests/ -v (~2572 tests)
...
just db-load 91st_2025-2026                  # load one session into PostgreSQL (DB2)
just db-load-all                             # load all sessions + ALEC corpus
just db-load-alec                            # load ALEC model legislation corpus
...
## Web / Database (DB1 + DB2)

Django project at `src/web/` for PostgreSQL-backed data access. The scraper continues writing CSVs; the DB2 loader imports them into PostgreSQL.
...
Justfile recipes: `just db-up` (start PostgreSQL), `just db-down` (stop), `just db-migrate` (apply migrations), `just db-makemigrations` (generate), `just db-load` (load one session — DB2), `just db-load-all` (load all sessions + ALEC), `just db-load-alec` (ALEC corpus only), `just db-admin` (runserver), `just db-createsuperuser`, `just db-shell` (psql), `just django <cmd>` (passthrough), `just test-web` (Django tests).
