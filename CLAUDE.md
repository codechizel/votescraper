...
- Canonical ideal points: tiered quality gate (ADR-0110) — Tier 1 (converged, R-hat < 1.10), Tier 2 (point estimates credible, R-hat < 2.50 + party separation d > 1.5), Tier 3 (fall back to 1D). Routing preference: Hierarchical 2D Dim 1 (if converged) → Flat 2D Dim 1 → 1D IRT, **validated by W-NOMINATE cross-validation gate (ADR-0123)** — if a different IRT dimension correlates better with W-NOMINATE Dim 1 by > 0.10, swap to the better dimension (catches 6/28 sessions where party-pooling prior distorts Dim 1). 2D IRT adaptive tuning for supermajority chambers (ADR-0112): N_TUNE doubles to 4000 when majority > 70%, beta init from PCA loadings, `--contested-only` flag. **All downstream phases that use IRT scores must load canonical ideal points** (Phase 06 routing output), not raw Phase 05 — Phase 15 (prediction) uses `_load_canonical_irt()` for this (ADR-0121).
- **PCA axis instability (OPEN, ADR-0118):** In 7/14 Senate sessions (78th-83rd, 88th), PC1 captures intra-Republican factionalism rather than the party divide. Current auto-detection via quality gates (R1-R7) and W-NOMINATE cross-validation gate (ADR-0123) catches most failures but still misroutes 6/28 sessions. PCA rotation methods (varimax, promax, oblimin, Procrustes) evaluated and rejected — they don't solve variance-ordering problems. Planned fix: manual override file (`analysis/pca_overrides.yaml`) for ambiguous sessions + W-NOMINATE gate demoted to diagnostic. See `docs/pca-ideology-axis-instability.md` and `docs/pca-rotation-and-human-intervention.md`.
...
- ADRs: `docs/adr/README.md` (126 decisions)
