...
- Canonical ideal points: tiered quality gate (ADR-0110) — Tier 1 (converged, R-hat < 1.10), Tier 2 (point estimates credible, R-hat < 2.50 + party separation d > 1.5), Tier 3 (fall back to 1D). Routing preference: Hierarchical 2D Dim 1 (if converged) → Flat 2D Dim 1 → 1D IRT, with **manual PCA overrides** (`analysis/pca_overrides.yaml`) for 8 problematic sessions where automated routing selects the wrong dimension. W-NOMINATE gate (ADR-0123) demoted to diagnostic-only — computes cross-validation correlations but no longer auto-swaps. 2D IRT adaptive tuning for supermajority chambers (ADR-0112): N_TUNE doubles to 4000 when majority > 70%, beta init from PCA loadings, `--contested-only` flag. **All downstream phases that use IRT scores must load canonical ideal points** (Phase 06 routing output), not raw Phase 05 — Phase 15 (prediction) uses `_load_canonical_irt()` for this (ADR-0121).
- **PCA axis instability (MANAGED, ADR-0118):** In 7/14 Senate sessions (78th-83rd, 88th), PC1 captures intra-Republican factionalism rather than the party divide. Managed via manual PCA overrides (`analysis/pca_overrides.yaml`) for the 8 affected sessions + automated `detect_ideology_pc()` for unambiguous sessions. PCA rotation methods (varimax, promax, oblimin, Procrustes) evaluated and rejected. W-NOMINATE gate demoted to diagnostic. See `docs/pca-rotation-and-human-intervention.md`.
