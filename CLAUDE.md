...
- Canonical ideal points: 3-layer routing (ADR-0109, ADR-0110). Layer 1: **manual PCA overrides** (`analysis/pca_overrides.yaml`) for 4 problematic sessions (79th-80th, 82nd-83rd Senate) — highest priority, early return. Layer 2: horseshoe detection. Layer 3: tiered convergence — Tier 1 (converged, R-hat < 1.10), Tier 2 (point estimates credible, R-hat < 2.50 + party separation d > 1.5), Tier 3 (fall back to 1D). Routing preference: H2D Dim 1 → Flat 2D Dim 1 → 1D IRT. W-NOMINATE gate removed (ADR-0127: shares same variance-ordering vulnerability as PCA). Phase 16 (W-NOMINATE) is standalone publication validation, not a routing input. 2D IRT adaptive tuning for supermajority chambers (ADR-0112): N_TUNE doubles to 4000 when majority > 70%, beta init from PCA loadings, `--contested-only` flag. **All downstream phases that use IRT scores must load canonical ideal points** (Phase 06 routing output), not raw Phase 05 — Phase 15 (prediction) uses `_load_canonical_irt()` for this (ADR-0121).
- **PCA axis instability (MANAGED, ADR-0118):** In 4/14 Senate sessions (79th-80th, 82nd-83rd), PC1 captures intra-Republican factionalism rather than the party divide even after contested-only filtering. Managed via manual PCA overrides (`analysis/pca_overrides.yaml`) for those 4 sessions + automated `detect_ideology_pc()` for all others. Four previous overrides (78th, 81st, 84th, 88th Senate) removed after re-audit at `CONTESTED_THRESHOLD=0.10` — see `docs/pca-dimension-audit-2026.md`. PCA rotation methods evaluated and rejected. W-NOMINATE shares same vulnerability (ADR-0127).
...
- ADRs: `docs/adr/README.md` (128 decisions)
- Design docs: `analysis/design/README.md`
- Deep dives: `docs/*.md` (search by topic name)
- Known issue: `docs/pca-ideology-axis-instability.md` — PC1 ≠ ideology in 4/14 Senate sessions (79th-80th, 82nd-83rd; re-audited at 0.10 threshold, see `docs/pca-dimension-audit-2026.md`)
