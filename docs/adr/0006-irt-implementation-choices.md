...
- PCA anchors assume PCA and IRT agree on who is most extreme within each party. In supermajority chambers, the most extreme PC1 value overall may belong to a majority-party rebel whose voting pattern resembles the minority (dimension collapse). The party-aware selection avoids this by picking extremes within each party separately. Same PCA-IRT correlation validation applies.
